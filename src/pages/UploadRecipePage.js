...
function UploadRecipePage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const token = localStorage.getItem("token");
    if (!token) return setShowLoginDialog(true);
    if (!image) return alert("Please upload an image.");
  ...
  return (
    <>
      <Dialog open={showLoginDialog} onClose={() => navigate("/")} PaperProps={{ sx: { borderRadius: 3, px: 4, py: 2, textAlign: "center" } }}>
        <DialogTitle sx={{ fontSize: "18px", fontWeight: 600 }}>You must be logged in to upload a recipe.</DialogTitle>
        <DialogActions sx={{ justifyContent: "center" }}>
          <Button variant="contained" sx={{ bgcolor: "#ff6600" }} onClick={() => navigate("/")}>OK</Button>
        </DialogActions>
      </Dialog>

      <Dialog open={showSuccessDialog} onClose={() => navigate("/")} PaperProps={{ sx: { borderRadius: 3, px: 4, py: 2, textAlign: "center" } }}>
        <DialogTitle sx={{ fontSize: "18px", fontWeight: 600 }}>Recipe uploaded successfully!</DialogTitle>
        <DialogActions sx={{ justifyContent: "center" }}>
          <Button variant="contained" sx={{ bgcolor: "#ff6600" }} onClick={() => navigate("/")}>OK</Button>
        </DialogActions>
      </Dialog>
        ...
        <Box sx={styles.overlay}>
          <Box sx={styles.wrapper}>
            <Card sx={styles.card}>
              <CardContent>
                <Box sx={{ textAlign: "center" }}>
                  <img src="/instaFood_logo.png" alt="Logo" style={styles.logo} />
                  <Typography variant="h5" sx={styles.title}>Upload New Recipe</Typography>
                </Box>
                  ...
                  <div style={styles.imageUpload}>
                    ...
                    <label htmlFor="image-upload" style={styles.uploadBox}>
                      {previewUrl ? (
                        ...
                        <Box sx={styles.uploadPlaceholder}>
                          <ImageOutlined sx={{ fontSize: 48, color: "#aaa" }} />
                          <Typography variant="body1" sx={{ mt: 1 }}>Click to upload recipe image</Typography>
                        </Box>
                      )}
                    </label>
                  </div>
